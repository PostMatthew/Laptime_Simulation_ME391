...
approxMinRad = 15; %m, approximated from endurance track
...
Car = buildCar();
LapTimeStruct = GetLapTime(Car,track);
LapTimeStruct = GetEnergy(LapTimeStruct);
message_1L = ["Energy consumed on one lap:" LapTimeStruct.Energy(end)/(3600) "Wh"]; % energy for one lap of track
message_2L = ["Energy consumed on one lap:" 2*LapTimeStruct.Energy(end)/(3600) "Wh"]; % energy for one lap of track
disp(message_1L)
disp(message_2L)

figure()
hold on
plot(LapTimeStruct.t_sim_vect,LapTimeStruct.Power)
...
function engine_parameters_out = LoadEngine()
    ...
    engine_parameters_out.Tmax = 140; %from data sheet, maximum torque 
...
function LapTimeStruct = BoolAssign(Car,LapTimeStruct,track)
    % Boolean logic to decide acceleration of car
    if LapTimeStruct.bool_turn
        LapTimeStruct.S_double_dot_car = 0;
    end
    if LapTimeStruct.bool_to_fast
    ...
    if (LapTimeStruct.S_dot_car >=track.S(length(track.S)))
        LapTimeStruct.S_dot_car = 30.1; % m/s
    end
end

function LapTimeStruct = GetLapTime(Car,track)
    % Initialize needed scalars and vectors
    ...
    while LapTimeStruct.S_car<=track.S(length(track.S))
        ...
        LapTimeStruct = BoolAssign(Car,LapTimeStruct,track);
...
function LapTimeStruct = GetEnergy(LapTimeStruct)
    LapTimeStruct.Power = LapTimeStruct.Omega_m_vect.*LapTimeStruct.Tm_vect; 
